...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe(device2_topic)  # Subscribe to Device 2's channel upon successful connection
    send_public_key()

def on_message(client, userdata, msg):
    global shared_key, key_generated
    #--delete this part later
    if msg.topic == device2_topic:
...
def send_public_key():
    client.publish(device1_topic, str(dh_pubkey))  # Publish Device 1's public key in its channel
    client.publish(device2_topic, "Hallo")  # Publish "Hallo" on Device 2's channel

def write_shared_key():
    with open('device1_shared_key.txt', 'w') as file:
        file.write(shared_key)
        print("Device 1 shared key written to file.")
